...
def job_spec_to_sbatch(
    ...
    if job_spec.environment:
        ...
        for k, v in job_spec.environment.items():
            ...
            if job_spec.container:
                # Also set APPTAINERENV_ and SINGULARITYENV_ prefixes for container compatibility
    ...
    # Build core command (handling container wrapping if container is specified)
    cmd_parts = []
    if job_spec.container:
    ...
    # Wrap with launcher if specified (e.g. srun, mpirun)
...
def _parse_slurm_duration(val: str) -> int:
...
def _parse_slurm_memory(val: str) -> int:
...
# Read an sbatch script file and parse it into a structured JobSpec.
    ...
    for line in lines:
        ...
        if stripped.startswith("#SBATCH"):
            ...
            if opt in ("job-name", "J"):
                ...
                if val:
                    duration = _parse_slurm_duration(val)
                ...
                if val:
                    memory = _parse_slurm_memory(val)
            else:
...
# convert a slurm script file to a JobSpec.
def sbatch_file_to_job_spec(file_path: str) -> compute_models.JobSpec:
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        content = f.read()
    return sbatch_to_job_spec(content)
