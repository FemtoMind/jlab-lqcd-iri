...
class ResourceSpec(IRIBaseModel):
    ...
    node_count: int | None = Field(
        default=None, ge=1, description="Number of compute nodes to allocate", example=2
    )
    process_count: int | None = Field(
        default=None, ge=1, description="Total number of processes to launch", example=64
    )
    processes_per_node: int | None = Field(
        default=None, ge=1, description="Number of processes to launch per node", example=32
    )
    cpu_cores_per_process: int | None = Field(
        default=None, ge=1, description="Number of CPU cores to allocate per process", example=2
    )
    gpu_cores_per_process: int | None = Field(
        default=None, ge=1, description="Number of GPU cores to allocate per process", example=1
    )
    exclusive_node_use: StrictBool = Field(
        default=True,
        description="Whether to request exclusive use of allocated nodes",
        example=True,
    )
    memory: int | None = Field(
        default=None, ge=1, description="Amount of memory to allocate in bytes", example=17179869184
    )
...
class JobAttributes(IRIBaseModel):
    ...
    duration: int | None = Field(
        default=None, description="Duration in seconds", ge=1, examples=[30, 60, 120]
    )
...
class VolumeMount(IRIBaseModel):
    ...
    read_only: StrictBool = Field(
        default=True, description="Whether the mount should be read-only", example=True
    )
...
class Container(IRIBaseModel):
    ...
    volume_mounts: list[VolumeMount] = Field(
        default_factory=list, description="List of volume mounts for the container"
    )
...
class JobSpec(IRIBaseModel):
    ...
    container: Container | None = Field(
        default=None, description="Container specification for containerized execution"
    )
    ...
    name: str | None = Field(
        default=None, min_length=1, description="Name of the job", example="my-job"
    )
    inherit_environment: StrictBool = Field(
        default=True,
    ...
    resources: ResourceSpec | None = Field(
        default=None, description="Resource requirements for the job"
    )
...
class JobStatus(IRIBaseModel):
    ...
    message: str | None = Field(
        default=None, description="Human-readable status message", example="Job is waiting in queue"
    )
    exit_code: int | None = Field(
        default=None, description="Process exit code if the job has finished", example=0
    )
    meta_data: dict[str, object] | None = Field(
        default=None, description="Backend-specific metadata associated with the job status"
    )
...
class Job(IRIBaseModel):
    ...
    job_spec: JobSpec | None = Field(
        default=None, description="Specification used to create the job"
    )
